...
都市の"いま"を観測するダッシュボード - 日本の主要 6 都市の天気・大気質を 1 画面で可視化する Web アプリケーション
...
- [ ] **F1-1**: 6 都市のボタン切り替え（東京/大阪/名古屋/札幌/福岡/那覇）
- [ ] **F1-2**: 既定都市は Tokyo
...
- [ ] **NFR-Q4**: E2E テスト（最低 1 本: 都市切替 → ダッシュボード表示）
...
#### 画面 A: Dashboard（単一ページ）

**目的**: 1 画面で主要情報を把握できること

**主要要素**:

- サイドバー（都市切り替え・ダークモード・プロフィール）
- 天気カード（現在値）
- 天気チャート（24h / 7d）
- 大気質カード（PM2.5 ほか）
- 大気質チャート（24h / 5d）
...
- 都市情報ヘッダー（都市名・国・現地時刻）
- 天気カード（気温/体感温度/湿度/風速/降水確率）
- 時系列チャート（24h/7d 切替、AQ は 5d）
- 大気質カード（PM2.5/PM10/NO2/O3）
...
#### 画面 C: Compare Mode（将来）

比較モードは MVP 後に検討（現状未実装）。

...
**注記**: 現状の MVP は固定 6 都市のダッシュボードのみ実装済み。都市検索/比較は将来対応のため、本チェックリストの該当項目はスキップ対象。
...
3. 天気チャート（24h → 7d）
4. 大気質の現在値 → チャート
5. 地図表示（マーカー + クレジット + 降水レイヤー）
6. 仕上げ（レスポンシブ・エラー/ローディング）
